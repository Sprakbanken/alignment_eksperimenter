...
if __name__ == "__main__":
    ...
    for (lang1, lang2), group in gd.groupby(["lang_lang_1", "lang_lang_2"]):
        for mode in MODES:
            ...
            positive_pairs = [(tup.fulltext_joined_lang_1, tup.fulltext_joined_lang_2) for tup in group[group.annotation == "Parallell"].itertuples(index=False)]
            negative_pairs = [(tup.fulltext_joined_lang_1, tup.fulltext_joined_lang_2) for tup in group[group.annotation == "Not parallell"].itertuples(index=False)]
            almost_parallel_pairs = [(tup.fulltext_joined_lang_1, tup.fulltext_joined_lang_2) for tup in group[group.annotation == "Almost parallel"].itertuples(index=False)]
            if mode == "strict":
                negative_pairs = negative_pairs + almost_parallel_pairs
            else:
                positive_pairs = positive_pairs + almost_parallel_pairs

            logger.info("Group size: %d, positive pairs: %d, negative pairs: %d", len(group), len(positive_pairs), len(negative_pairs))
            ...
            for embedding_model in EMBEDDING_MODEL_LIST:
                ...
                for aggregation_strategy in AGGREGATION_STRATEGY:
                    ...
                    for threshold in THRESHOLDS:
                        ...
                        confusion_matrix = compare_alignment(config, positive_pairs=positive_pairs, negative_pairs=negative_pairs, filename_prefix=f"{mode}_{lang1}_{lang2}")
                        scores = calculate_scores(confusion_matrix)
                        scores_path = model_output_dir / f"scores_{aggregation_strategy}_{threshold:.2f}_{lang1}_{lang2}_{mode}.json"
