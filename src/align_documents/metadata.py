...
HASH_FN = (
    hashlib.sha1
)  # "doc_hash" in the maalfrid dataset uses sha1, so we use it here too.
...
def generate_dataset_metadata(
    dataset_path: Path, write_filepath: Path | None = None
) -> pd.DataFrame:
    if write_filepath is None:
        write_filepath = (
            dataset_path / DATASET_METADATA_DIRNAME / DATASET_METADATA_FILENAME
        )
    ...
    format = padataset.JsonFileFormat(
        ...
        parse_options=pajson.ParseOptions(
            explicit_schema=DATASET_METADATA_SCHEMA, unexpected_field_behavior="ignore"
        ),
    )

    dataset = padataset.dataset(
        dataset_path, format=format, schema=DATASET_METADATA_SCHEMA
    )
...
def _generate_hashtree(
    ...
    def _hash_hashlist(hashlist: list[str], type_: Literal["leaf", "inode"]) -> str:
    ...
    if not level_keys:
        ...
        return {
            ...
            KEY_BRANCHES: {
                leaf_name: {KEY_HASH: leaf_hash, KEY_LEVEL: LEVEL_LEAF}
                for leaf_name, leaf_hash in zip(leaf_names, leaf_hashes)
            },
        }
...
def generate_hashtree(
    ...
    hashtree_internal = _generate_hashtree(
        docs, level_keys, leaf_name_key, leaf_hash_key
    )
...
def get_dataset_hashtree(
    ...
    if hashtree_file.is_file():
        ...
        answer = input(
            "Could not find dataset hashtree. Generating it could take a long time. Do it now (else skip)? [Y/n]: "
        )
    ...
    hashtree = generate_hashtree(
        dataset_metadata, level_keys, leaf_name_key, leaf_hash_key
    )

    if not hashtree:
        logger.warning(
            "Received empty hashtree - will not write. Dataset path: %s", dataset_path
        )
        return None
...
class AlignmentRun:
    def __init__(
        ...
        if input_hashtree := get_dataset_hashtree(config.data_dir):
            self.metadata_dict["datasets"]["input"] = (
                self.create_metadata_dataset_entry(
                    hashtree=input_hashtree,
                    hashtree_path=config.data_dir
                    / DATASET_METADATA_DIRNAME
                    / DATASET_HASHTREE_FILENAME,
                    metadata_path=config.data_dir
                    / DATASET_METADATA_DIRNAME
    ...
    def extend_pipeline_input_docs(
        ...
        self._pipeline_input_docs_list.append(docs[DATASET_METADATA_SCHEMA.names])
    ...
    def write(self):
        ...
        # Filenames
        pipeline_input_hashtree_file = (
            self.metadata_dir / "pipeline_input_hashtree.json"
        )
        ...
        if self.pipeline_input_docs.empty:
            ...
            logger.debug(
                "Hashtree written for pipeline input: %s", pipeline_input_hashtree_file
            )
